...
internal class BleUnclaimDeviceSingle(rxFit: RxFit, val bleDevice: BleDevice?, val deviceAddress: String?, timeout: Long?, timeUnit: TimeUnit?) : BaseSingle<Status>(rxFit, timeout, timeUnit) {

    override fun onGoogleApiClientReady(apiClient: GoogleApiClient?, subscriber: SingleEmitter<Status>) {
        val resultCallback = SingleResultCallBack.get(subscriber)
